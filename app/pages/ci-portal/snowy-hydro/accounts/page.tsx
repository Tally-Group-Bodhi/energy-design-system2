...
import {
  ...
  colours,
  money,
} from "../components/SnowyPortalShell";
...
function AccountHeader() {
  return (
    <div className="mb-density-lg flex flex-wrap items-start justify-between gap-density-lg">
      <div>
        <h1 className="flex items-center gap-density-sm text-density-3xl font-semibold text-snowy-gray-900">
          Account: {ACCOUNT.id}
          <Icon name="chevron_right" size={ICON_LG} className="text-snowy-teal-700" />
        </h1>
        <p className="mt-density-xs text-density-base text-snowy-gray-600">
          {ACCOUNT.name}
        </p>
      </div>
      <p className="text-density-lg font-medium text-snowy-teal-700">
...
function AccountTabs({
  ...
  return (
    <div
      ...
      className="mb-density-lg flex w-full gap-0 overflow-x-auto border-b border-snowy-gray-200"
    >
        ...
        return (
          <button
            ...
            className={`shrink-0 border-b-2 px-density-lg py-density-md text-density-sm font-medium uppercase tracking-wide transition-colors ${
              active
                ? "border-snowy-teal-700 text-snowy-teal-700"
                : "border-transparent text-snowy-gray-600 hover:text-snowy-gray-900"
            }`}
          >
            {tab.label}
          </button>
...
function DetailRow({
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-density-sm gap-y-1 py-density-sm">
      <dt className="min-w-[10rem] text-density-sm font-semibold text-snowy-gray-900">
        {label}:
      </dt>
      <dd className="text-density-sm text-snowy-gray-600">{children}</dd>
    </div>
  );
}
...
function SectionCard({
  ...
  return (
    <Card className={`border-snowy-gray-200 bg-white ${className}`}>
      <CardContent className="p-density-xl pt-density-xl">
        <h2 className="mb-density-md text-density-lg font-semibold text-snowy-teal-700">
          {title}
        </h2>
        {children}
      </CardContent>
...
function DataTileArt({ icon }: { icon: string }) {
  return (
    <span
      className="grid h-14 w-14 shrink-0 place-items-center rounded-full bg-snowy-teal-50"
      aria-hidden="true"
    >
      <Icon name={icon} size={28} className="text-snowy-teal-700" />
    </span>
  );
}
...
function DataTile({
  ...
  return (
    <button
      type="button"
      className="flex w-full items-stretch gap-density-md rounded-density-lg border border-snowy-gray-200 bg-white p-density-lg text-left transition-colors hover:border-snowy-teal-700/40 hover:bg-snowy-teal-50"
    >
      <div className="min-w-0 flex-1">
        <div className="mb-density-sm flex items-center gap-density-sm">
          <Icon name={icon} size={ICON_MD} className="text-snowy-gray-600" />
          <span className="truncate text-density-base font-semibold text-snowy-gray-900">
            {title}
          </span>
        </div>
        <p className="text-density-sm text-snowy-gray-600">{description}</p>
      </div>
...
function MonthlyConsumptionChart() {
  return (
    <Card className="border-snowy-gray-200 bg-white">
      <CardContent className="p-density-xl pt-density-xl">
        <div className="mb-density-md flex items-center justify-between gap-density-md">
          <h3 className="text-density-lg font-medium text-snowy-teal-950">
            Monthly consumption
          </h3>
          <button
            type="button"
            aria-label="Expand monthly consumption"
            className="flex h-9 w-9 items-center justify-center rounded-density-md text-snowy-gray-600 transition-colors hover:bg-snowy-teal-50 hover:text-snowy-teal-950"
          >
        ...
        <ul className="mt-density-sm flex flex-wrap items-center justify-center gap-x-density-lg gap-y-density-xs">
          {[
            { label: "Off-peak", colour: PURPLE },
            { label: "Peak", colour: ACCENT },
            { label: "Generation", colour: BLUE },
          ].map((item) => (
            <li
              key={item.label}
              className="flex items-center gap-density-sm text-density-xs text-snowy-gray-600"
            >
              <span
        ...
        <dl>
          ...
          <DetailRow label="Parent account">
            <Checkbox
              ...
              className="accent-[var(--color-snowy-teal-950)]"
            />
          </DetailRow>
    ...
    <div className="grid grid-cols-1 gap-density-lg xl:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)]">
        ...
        <CardContent className="p-density-xl pt-density-xl">
          <div className="relative mb-density-md">
            <Icon
              name="search"
              size={ICON_MD}
              className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-snowy-gray-600"
            />
            <Input
              value={query}
              ...
              className="h-11 rounded-density-md border-snowy-gray-300 pl-10"
            />
          </div>
              ...
              <button
                ...
                className={`flex w-full items-center gap-density-sm rounded-density-md px-density-md py-density-sm text-left transition-colors ${
                  selectedId === ACCOUNT.id
                    ? "bg-snowy-teal-50 text-snowy-teal-950"
                    : "text-snowy-gray-600 hover:bg-snowy-gray-100"
                }`}
              >
                ...
                <span className="tabular-nums text-density-sm text-snowy-gray-600">
              ...
              <li key={child.id} className="pl-density-xl">
                <button
                  ...
                  className={`flex w-full items-center gap-density-sm rounded-density-md px-density-md py-density-sm text-left transition-colors ${
                    selectedId === child.id
                      ? "bg-snowy-teal-50 text-snowy-teal-950"
                      : "text-snowy-gray-900 hover:bg-snowy-gray-100"
                  }`}
                >
                  <Icon name="chevron_right" size={ICON_SM} className="text-snowy-gray-600" />
                  <Icon name="person" size={ICON_MD} className="text-snowy-gray-600" />
      ...
      <div className="flex flex-col gap-density-lg">
        <SectionCard title={`Account: ${ACCOUNT.id}`}>
          <p className="mb-density-md text-density-base font-semibold text-snowy-gray-900">
            {ACCOUNT.name}
          </p>
          <dl>
            <DetailRow label="Account status">{ACCOUNT.status}</DetailRow>
    ...
    <div className="space-y-density-xl">
      {DATA_SECTIONS.map((section) => (
        <section key={section.title}>
          <h2 className="mb-density-md text-density-lg font-semibold text-snowy-gray-900">
            {section.title}
          </h2>
  ...
  return (
    <Card className="border-snowy-gray-200 bg-white">
      <CardContent className="flex flex-col items-center justify-center gap-density-sm p-density-xxl pt-density-xxl text-center">
        <Icon name="construction" size={ICON_LG} className="text-snowy-teal-700" />
        <p className="text-density-lg font-medium text-snowy-teal-950">
          {label} is coming next
        </p>
        <p className="max-w-md text-density-sm text-snowy-gray-600">
...
export default function SnowyHydroAccountDetailsPage() {
  const [tab, setTab] = React.useState<TabValue>("general");

  return (
    <SnowyPortalShell activeNavId="accounts-list">
      <Breadcrumb className="mb-density-md">
        <BreadcrumbList className="text-density-sm text-snowy-gray-600">
          <BreadcrumbItem>
            <BreadcrumbLink asChild>
              <Link href={HOME_HREF} className="text-snowy-teal-700 hover:text-snowy-teal-950">
                Home
              </Link>
            </BreadcrumbLink>
          </BreadcrumbItem>
          ...
          <BreadcrumbItem>
            <BreadcrumbLink asChild>
              <Link
                href="/pages/ci-portal/snowy-hydro/accounts"
                className="text-snowy-teal-700 hover:text-snowy-teal-950"
              >
                Accounts
              </Link>
            </BreadcrumbLink>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbPage className="text-snowy-gray-600">
              Account details
            </BreadcrumbPage>
          </BreadcrumbItem>
        </BreadcrumbList>
