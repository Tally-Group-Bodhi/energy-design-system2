...
const TONE_CLASS = {
  orange: "bg-snowy-teal-50 text-snowy-teal-700",
  purple: "bg-snowy-teal-50 text-snowy-teal-950",
  success: "bg-[color-mix(in_srgb,var(--color-snowy-success)_12%,white)] text-[var(--color-snowy-success)]",
} as const;

function ParticipantCard({ participant }: { participant: Participant }) {
  return (
    <article className="rounded-density-lg border border-snowy-gray-200 bg-white p-density-lg">
      <div className="mb-density-md flex items-center gap-density-md">
        ...
        <h3 className="truncate text-density-base font-semibold uppercase tracking-wide text-snowy-gray-900">
          {participant.name}
        </h3>
      </div>
      <dl className="space-y-density-sm text-density-sm">
        <div className="flex items-center justify-between gap-density-md">
          <dt className="text-snowy-gray-600">Next invoice due</dt>
          <dd className="tabular-nums text-snowy-gray-900">
            {participant.nextInvoiceDue}
          </dd>
        </div>
        <div className="flex items-center justify-between gap-density-md">
          <dt className="text-snowy-gray-600">Due amount</dt>
          <dd className="tabular-nums text-snowy-gray-900">
            {money.format(participant.dueAmount)}
          </dd>
        </div>
        <div className="flex items-center justify-between gap-density-md">
          <dt className="text-snowy-gray-600">Disputed amount</dt>
          <dd className="tabular-nums text-snowy-gray-900">
...
export default function NetworkSettlementHomePage() {
  return (
    <SnowyPortalShell activeNavId="network-home">
      ...
      <Card className="border-snowy-gray-200 bg-white">
        <CardContent className="p-density-xl pt-density-xl">
          <h2 className="mb-density-lg text-density-lg font-semibold text-snowy-teal-950">
            Participant
          </h2>
