...
function SettlementWeekCard({ card }: { card: SettlementCard }) {
  return (
    <Link
      ...
      className="block rounded-density-lg border border-ea-gray-200 bg-white p-density-lg transition-colors hover:border-ea-green-600/40"
    >
      <div className="mb-density-md flex items-start gap-density-md">
        <span className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-ea-gray-100 text-density-sm font-semibold text-ea-gray-600">
          {card.code}
        </span>
        <div className="min-w-0 flex-1">
          <p className="text-density-base font-semibold text-ea-green-950">
            {card.title}
          </p>
          <p className="text-density-sm text-ea-green-600">{card.due}</p>
        </div>
        {card.complete && (
          <span className="grid h-6 w-6 place-items-center rounded-full bg-[var(--color-ea-success)] text-white">
            <Icon name="check" size={ICON_SM} />
          </span>
        )}
      </div>
      <div className="space-y-density-xs text-density-sm text-ea-gray-600">
        <p className="tabular-nums">
          {card.caseId} | {card.period}
        </p>
        <p>
          Settlement:{" "}
          <span className="tabular-nums text-ea-gray-900">
    ...
    <div className="grid items-start gap-density-xl lg:grid-cols-[auto_1fr]">
      <div>
        <Calendar
          ...
          className="ea-calendar rounded-density-md border border-ea-gray-200 bg-white [--cell-size:2.75rem]"
        />
        <p className="mt-density-md flex items-center gap-density-xs text-density-xs text-ea-gray-600">
          <span
            aria-hidden="true"
            className="h-1.5 w-1.5 rounded-full bg-ea-green-600"
          />
      ...
      <div>
        ...
        {dayCards.length > 0 ? (
          ...
          <div className="flex items-center gap-density-sm rounded-density-lg border border-dashed border-ea-gray-300 p-density-lg text-density-sm text-ea-gray-600">
            <Icon
              name="event_available"
              size={ICON_MD}
              className="text-ea-gray-600"
            />
  ...
  return (
    <EaPortalShell activeNavId="settlements-home">
      ...
      <div className="mb-density-lg flex flex-wrap items-end justify-between gap-density-lg">
        <div>
          <h1 className="text-density-3xl font-semibold text-ea-gray-900">
            Settlements
          </h1>
        ...
        <div className="flex flex-wrap items-end gap-density-md">
          ...
          <button
            type="button"
            disabled
            className="inline-flex h-11 items-center gap-density-sm rounded-density-md border border-ea-gray-300 px-density-lg text-density-sm text-ea-gray-600 opacity-60"
          >
      ...
      <Card className="border-ea-gray-200 bg-white">
        <CardContent className="p-density-xl pt-density-xl">
          <div
            ...
            className="mb-density-lg flex border-b border-ea-gray-200"
          >
            {(
              [
