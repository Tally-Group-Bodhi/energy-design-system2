...
const TONE_CLASS = {
  orange: "bg-ea-green-50 text-ea-green-600",
  purple: "bg-ea-green-50 text-ea-green-950",
  success: "bg-[color-mix(in_srgb,var(--color-ea-success)_12%,white)] text-[var(--color-ea-success)]",
} as const;

function ParticipantCard({ participant }: { participant: Participant }) {
  return (
    <article className="rounded-density-lg border border-ea-gray-200 bg-white p-density-lg">
      <div className="mb-density-md flex items-center gap-density-md">
        ...
        <h3 className="truncate text-density-base font-semibold uppercase tracking-wide text-ea-gray-900">
          {participant.name}
        </h3>
      </div>
      <dl className="space-y-density-sm text-density-sm">
        <div className="flex items-center justify-between gap-density-md">
          <dt className="text-ea-gray-600">Next invoice due</dt>
          <dd className="tabular-nums text-ea-gray-900">
            {participant.nextInvoiceDue}
          </dd>
        </div>
        <div className="flex items-center justify-between gap-density-md">
          <dt className="text-ea-gray-600">Due amount</dt>
          <dd className="tabular-nums text-ea-gray-900">
            {money.format(participant.dueAmount)}
          </dd>
        </div>
        <div className="flex items-center justify-between gap-density-md">
          <dt className="text-ea-gray-600">Disputed amount</dt>
          <dd className="tabular-nums text-ea-gray-900">
...
export default function NetworkSettlementHomePage() {
  return (
    <EaPortalShell activeNavId="network-home">
      ...
      <Card className="border-ea-gray-200 bg-white">
        <CardContent className="p-density-xl pt-density-xl">
          <h2 className="mb-density-lg text-density-lg font-semibold text-ea-green-950">
            Participant
          </h2>
