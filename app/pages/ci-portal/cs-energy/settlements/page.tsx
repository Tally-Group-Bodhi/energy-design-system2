...
function SettlementWeekCard({ card }: { card: SettlementCard }) {
  return (
    <Link
      ...
      className="block rounded-density-lg border border-cs-gray-200 bg-white p-density-lg transition-colors hover:border-cs-blue-600/40"
    >
      <div className="mb-density-md flex items-start gap-density-md">
        <span className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-cs-gray-100 text-density-sm font-semibold text-cs-gray-600">
          {card.code}
        </span>
        <div className="min-w-0 flex-1">
          <p className="text-density-base font-semibold text-cs-blue-800">
            {card.title}
          </p>
          <p className="text-density-sm text-cs-blue-600">{card.due}</p>
        </div>
        {card.complete && (
          <span className="grid h-6 w-6 place-items-center rounded-full bg-[var(--color-cs-success)] text-white">
            <Icon name="check" size={ICON_SM} />
          </span>
        )}
      </div>
      <div className="space-y-density-xs text-density-sm text-cs-gray-600">
        <p className="tabular-nums">
          {card.caseId} | {card.period}
        </p>
        <p>
          Settlement:{" "}
          <span className="tabular-nums text-cs-gray-900">
    ...
    <div className="grid items-start gap-density-xl lg:grid-cols-[auto_1fr]">
      <div>
        <Calendar
          ...
          className="cs-calendar rounded-density-md border border-cs-gray-200 bg-white [--cell-size:2.75rem]"
        />
        <p className="mt-density-md flex items-center gap-density-xs text-density-xs text-cs-gray-600">
          <span
            aria-hidden="true"
            className="h-1.5 w-1.5 rounded-full bg-cs-blue-600"
          />
      ...
      <div>
        ...
        {dayCards.length > 0 ? (
          ...
          <div className="flex items-center gap-density-sm rounded-density-lg border border-dashed border-cs-gray-300 p-density-lg text-density-sm text-cs-gray-600">
            <Icon
              name="event_available"
              size={ICON_MD}
              className="text-cs-gray-600"
            />
...
export default function SettlementsHomePage() {
  ...
  return (
    <CsPortalShell activeNavId="settlements-home">
      ...
      <div className="mb-density-lg flex flex-wrap items-end justify-between gap-density-lg">
        ...
        <div className="flex flex-wrap items-end gap-density-md">
          ...
          <button
            type="button"
            disabled
            className="inline-flex h-11 items-center gap-density-sm rounded-density-md border border-cs-gray-300 px-density-lg text-density-sm text-cs-gray-600 opacity-60"
          >
      ...
      <Card className="border-cs-gray-200 bg-white">
        <CardContent className="p-density-xl pt-density-xl">
          <div
            ...
            className="mb-density-lg flex border-b border-cs-gray-200"
          >
            {(
              [
