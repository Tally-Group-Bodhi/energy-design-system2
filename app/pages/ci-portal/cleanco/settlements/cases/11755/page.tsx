...
const HISTORY = [
  {
    ...
    colour: "var(--color-cleanco-cyan-300)",
  },
  {
    label: "Final Case 11576",
    due: "Due Fri 24-Apr-2026",
    colour: "var(--color-cleanco-success)",
  },
  {
    label: "Rev 0",
    due: "",
    colour: "var(--color-cleanco-gray-300)",
  },
  {
    label: "Wk 20 Rev 1 Case 11755",
    due: "Due Tue 11-Aug-2026",
    colour: "var(--color-cleanco-cyan-700)",
    current: true,
  },
  {
    label: "Wk 30 Rev 2",
  ...
  return (
    <CleanCoPortalShell activeNavId="settlements-home">
      ...
      <div
        ...
        className="mb-density-lg flex flex-wrap border-b border-cleanco-gray-200"
      >
        {CASE_TABS.map((item) => {
        ...
        <div className="grid grid-cols-1 gap-density-lg xl:grid-cols-2">
          <Card className="border-cleanco-gray-200 bg-white">
            <CardContent className="flex h-full flex-col p-density-xl pt-density-xl">
              <h2 className="text-density-lg font-semibold text-cleanco-cyan-950">
                Case History Wk:13-2026 22-Mar — 28-Mar
              </h2>
              <p className="mt-density-xs text-density-sm text-cleanco-gray-700">
                Started Mon 10-Aug-2026 09:14 · Completed Mon 10-Aug-2026 11:02
              </p>

              <ol className="relative mt-density-lg space-y-density-lg border-l-2 border-cleanco-gray-200 pl-density-lg">
                {HISTORY.map((item) => (
                  <li key={item.label} className="relative">
                    ...
                    <p
                      className={`text-density-sm font-semibold ${
                        item.current
                          ? "text-cleanco-cyan-950"
                          : "text-cleanco-gray-900"
                      }`}
                    >
                      {item.label}
                    </p>
                    {item.due && (
                      <p className="text-density-xs text-cleanco-gray-700">
                        {item.due}
                      </p>
                    )}
                  </li>
              ...
              <button
                ...
                style={{ backgroundColor: "var(--color-cleanco-cyan-950)" }}
              >
          ...
          <Card className="border-cleanco-gray-200 bg-white">
            <CardContent className="p-density-xl pt-density-xl">
              <h2 className="mb-density-md text-density-lg font-semibold text-cleanco-cyan-950">
                Comparison
              </h2>
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  {COMPARISON.map((row) => (
                    <TableRow
                      key={row.description}
                      className="border-cleanco-gray-200"
                    >
                      <TableCell className="font-medium text-cleanco-gray-900">
                        {row.description}
                      </TableCell>
                      <TableCell className="text-right tabular-nums text-cleanco-gray-700">
                        {row.aemo}
                      </TableCell>
                      <TableCell className="text-right tabular-nums text-cleanco-gray-700">
                        {row.billed}
          ...
          <Card className="border-cleanco-gray-200 bg-white xl:col-span-2">
            <CardContent className="p-density-xl pt-density-xl">
              <h2 className="mb-density-md text-density-lg font-semibold text-cleanco-cyan-950">
                Job details
              </h2>
              <ul className="space-y-density-md">
                {JOBS.map((job) => (
                  <li
                    key={job.title}
                    className="flex items-start gap-density-md rounded-density-lg border border-[var(--color-cleanco-success)]/30 bg-[color-mix(in_srgb,var(--color-cleanco-success)_8%,white)] p-density-lg"
                  >
                    <span className="mt-0.5 grid h-7 w-7 shrink-0 place-items-center rounded-full bg-[var(--color-cleanco-success)] text-white">
                      <Icon name="check" size={ICON_MD} />
                    </span>
                    <div>
                      <p className="text-density-base font-semibold text-cleanco-gray-900">
                        {job.title}
                      </p>
                      <p className="mt-density-xs text-density-sm text-cleanco-gray-700">
                        {job.detail}
