...
const CUSTOMERS = [
  "CleanCo — Swanbank Power Station",
  "CleanCo — Wivenhoe Power Station",
  "CleanCo — Western Downs",
  "CleanCo — Corporate Portfolio",
];
...
const COST_COLOURS = [ACCENT, CYAN_MID, CYAN_MARK, CYAN_LIGHT, NEUTRAL];
...
function RegionMark({ region, colour }: { region: string; colour: string }) {
  ...
  return (
    <svg
      ...
      aria-hidden="true"
    >
      <path d={AUSTRALIA_OUTLINE} fill="var(--color-cleanco-gray-300)" />
      <circle
        cx={mark.x}
        cy={mark.y}
        r={3.2}
        fill={colour}
        stroke="var(--color-cleanco-gray-25)"
        strokeWidth={1}
      />
    </svg>
  );
...
function ChartCard({
  ...
  const controlClass =
    "flex h-9 w-9 items-center justify-center rounded-density-md text-cleanco-gray-700 transition-colors hover:bg-cleanco-cyan-50 hover:text-cleanco-cyan-950";

  return (
    <>
      <Card className="flex flex-col border-cleanco-gray-200 bg-white">
        ...
        <CardContent className="pt-0">
          <h2 className="mb-density-md text-density-xl font-medium text-cleanco-cyan-950">
            {title}
          </h2>
      ...
      <Dialog open={expanded} onOpenChange={setExpanded}>
        <DialogContent className="max-w-[min(1200px,92vw)] rounded-density-lg border-cleanco-gray-200">
          <DialogHeader>
            <DialogTitle className="text-cleanco-cyan-950">{title}</DialogTitle>
          </DialogHeader>
...
function StatTile({
  ...
  return (
    <Card className="flex flex-col border-0 bg-cleanco-cyan-50">
      <CardContent className="flex flex-1 flex-col justify-center p-density-xl pt-density-xl">
        <div className="flex items-center justify-between gap-density-sm">
          <p className="truncate text-density-sm leading-none text-cleanco-cyan-700">
            {label}
          </p>
          <span className="grid h-7 w-7 shrink-0 place-items-center rounded-density-md border border-cleanco-cyan-300 bg-white">
            <Icon name={icon} size={ICON_MD} className="text-cleanco-cyan-700" />
          </span>
        </div>
        <p className="mt-density-md text-density-4xl font-semibold leading-none tracking-tight tabular-nums text-cleanco-cyan-950">
          {value}
        </p>
        {/* Cards without a delta still reserve its line, so the values stay on
            a common baseline across the row. */}
        {delta ? (
          <p className="mt-density-sm text-density-xs leading-none text-cleanco-cyan-700">
...
export default function CleanCoCiPortalPage() {
  const [customer, setCustomer] = React.useState(CUSTOMERS[0]);
  const [account, setAccount] = React.useState(ACCOUNTS[0]);

  const contextButton =
    "flex h-11 w-11 items-center justify-center rounded-density-md text-cleanco-gray-700 transition-colors hover:bg-cleanco-cyan-50 hover:text-cleanco-cyan-950";

  return (
    <CleanCoPortalShell activeNavId="home">
      ...
      <div className="mb-density-lg flex flex-wrap items-end gap-density-lg">
        ...
        <Link
          ...
          style={{
            backgroundColor: "var(--color-cleanco-cyan-950)",
            borderRadius: "9999px",
          }}
        >
          View account
        ...
        <div className="ml-auto flex items-center gap-density-xs">
          ...
          <button
            ...
            className={`${contextButton} text-cleanco-cyan-700 hover:text-cleanco-cyan-700`}
          >
          ...
          >
            ...
            <Card className="mb-density-lg border-cleanco-gray-200 bg-white">
              <CardContent className="p-density-xl pt-density-xl">
                <div className="mb-density-md flex flex-wrap items-center justify-between gap-density-md">
                  <h2 className="text-density-base font-medium text-cleanco-cyan-950">
                    Spot price $/MWh
                  </h2>
                  <p className="text-density-sm text-cleanco-gray-700">
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="min-w-0">
                          <p className="flex items-center gap-density-xs text-density-sm text-cleanco-gray-700">
                            {region.region}
                            {region.home && (
                              <>
                                ...
                                />
                                <span className="text-cleanco-cyan-700">home</span>
                              </>
                            )}
                          </p>
                          <p
            ...
            <div className="grid grid-cols-1 gap-density-lg pb-density-xl xl:grid-cols-2">
              <ChartCard
                ...
                legend={[
                  ...
                  { label: "Generation", colour: CYAN_MARK, dash: "0" },
                ]}
              >
