...
import {
  ...
  colours,
  money,
} from "../components/AmpolPortalShell";
...
function AccountHeader() {
  return (
    <div className="mb-density-lg flex flex-wrap items-start justify-between gap-density-lg">
      <div>
        <h1 className="flex items-center gap-density-sm text-density-3xl font-semibold text-ampol-gray-900">
          Account: {ACCOUNT.id}
          <Icon name="chevron_right" size={ICON_LG} className="text-ampol-red-600" />
        </h1>
        <p className="mt-density-xs text-density-base text-ampol-gray-600">
          {ACCOUNT.name}
        </p>
      </div>
      <p className="text-density-lg font-medium text-ampol-red-600">
...
function AccountTabs({
  ...
  return (
    <div
      ...
      className="mb-density-lg flex w-full gap-0 overflow-x-auto border-b border-ampol-gray-200"
    >
        ...
        return (
          <button
            ...
            className={`shrink-0 border-b-2 px-density-lg py-density-md text-density-sm font-medium uppercase tracking-wide transition-colors ${
              active
                ? "border-ampol-red-600 text-ampol-red-600"
                : "border-transparent text-ampol-gray-600 hover:text-ampol-gray-900"
            }`}
          >
            {tab.label}
          </button>
...
function DetailRow({
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-density-sm gap-y-1 py-density-sm">
      <dt className="min-w-[10rem] text-density-sm font-semibold text-ampol-gray-900">
        {label}:
      </dt>
      <dd className="text-density-sm text-ampol-gray-600">{children}</dd>
    </div>
  );
}
...
function SectionCard({
  ...
  return (
    <Card className={`border-ampol-gray-200 bg-white ${className}`}>
      <CardContent className="p-density-xl pt-density-xl">
        <h2 className="mb-density-md text-density-lg font-semibold text-ampol-red-600">
          {title}
        </h2>
        {children}
      </CardContent>
...
function DataTileArt({ icon }: { icon: string }) {
  return (
    <span
      className="grid h-14 w-14 shrink-0 place-items-center rounded-full bg-ampol-blue-50"
      aria-hidden="true"
    >
      <Icon name={icon} size={28} className="text-ampol-blue-700" />
    </span>
  );
}
...
function DataTile({
  ...
  return (
    <button
      type="button"
      className="flex w-full items-stretch gap-density-md rounded-density-lg border border-ampol-gray-200 bg-white p-density-lg text-left transition-colors hover:border-ampol-red-600/40 hover:bg-ampol-blue-50"
    >
      <div className="min-w-0 flex-1">
        <div className="mb-density-sm flex items-center gap-density-sm">
          <Icon name={icon} size={ICON_MD} className="text-ampol-gray-600" />
          <span className="truncate text-density-base font-semibold text-ampol-gray-900">
            {title}
          </span>
        </div>
        <p className="text-density-sm text-ampol-gray-600">{description}</p>
      </div>
...
function MonthlyConsumptionChart() {
  return (
    <Card className="border-ampol-gray-200 bg-white">
      <CardContent className="p-density-xl pt-density-xl">
        <div className="mb-density-md flex items-center justify-between gap-density-md">
          <h3 className="text-density-lg font-medium text-ampol-blue-700">
            Monthly consumption
          </h3>
          <button
            type="button"
            aria-label="Expand monthly consumption"
            className="flex h-9 w-9 items-center justify-center rounded-density-md text-ampol-gray-600 transition-colors hover:bg-ampol-blue-50 hover:text-ampol-blue-700"
          >
        ...
        <ul className="mt-density-sm flex flex-wrap items-center justify-center gap-x-density-lg gap-y-density-xs">
          {[
            { label: "Off-peak", colour: BLUE_LIGHT },
            { label: "Peak", colour: ACCENT },
            { label: "Generation", colour: BLUE_DEEP },
          ].map((item) => (
            <li
              key={item.label}
              className="flex items-center gap-density-sm text-density-xs text-ampol-gray-600"
            >
              <span
        ...
        <dl>
          ...
          <DetailRow label="Parent account">
            <Checkbox
              ...
              className="accent-[var(--color-ampol-blue-700)]"
            />
          </DetailRow>
    ...
    <div className="grid grid-cols-1 gap-density-lg xl:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)]">
        ...
        <CardContent className="p-density-xl pt-density-xl">
          <div className="relative mb-density-md">
            <Icon
              name="search"
              size={ICON_MD}
              className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-ampol-gray-600"
            />
            <Input
              value={query}
              ...
              className="h-11 rounded-density-md border-ampol-gray-300 pl-10"
            />
          </div>
              ...
              <button
                ...
                className={`flex w-full items-center gap-density-sm rounded-density-md px-density-md py-density-sm text-left transition-colors ${
                  selectedId === ACCOUNT.id
                    ? "bg-ampol-blue-50 text-ampol-blue-700"
                    : "text-ampol-gray-600 hover:bg-ampol-gray-100"
                }`}
              >
                ...
                <span className="tabular-nums text-density-sm text-ampol-gray-600">
              ...
              <li key={child.id} className="pl-density-xl">
                <button
                  ...
                  className={`flex w-full items-center gap-density-sm rounded-density-md px-density-md py-density-sm text-left transition-colors ${
                    selectedId === child.id
                      ? "bg-ampol-blue-50 text-ampol-blue-700"
                      : "text-ampol-gray-900 hover:bg-ampol-gray-100"
                  }`}
                >
                  <Icon name="chevron_right" size={ICON_SM} className="text-ampol-gray-600" />
                  <Icon name="person" size={ICON_MD} className="text-ampol-gray-600" />
      ...
      <div className="flex flex-col gap-density-lg">
        <SectionCard title={`Account: ${ACCOUNT.id}`}>
          <p className="mb-density-md text-density-base font-semibold text-ampol-gray-900">
            {ACCOUNT.name}
          </p>
          <dl>
            <DetailRow label="Account status">{ACCOUNT.status}</DetailRow>
    ...
    <div className="space-y-density-xl">
      {DATA_SECTIONS.map((section) => (
        <section key={section.title}>
          <h2 className="mb-density-md text-density-lg font-semibold text-ampol-gray-900">
            {section.title}
          </h2>
  ...
  return (
    <Card className="border-ampol-gray-200 bg-white">
      <CardContent className="flex flex-col items-center justify-center gap-density-sm p-density-xxl pt-density-xxl text-center">
        <Icon name="construction" size={ICON_LG} className="text-ampol-red-600" />
        <p className="text-density-lg font-medium text-ampol-blue-700">
          {label} is coming next
        </p>
        <p className="max-w-md text-density-sm text-ampol-gray-600">
...
export default function AmpolAccountDetailsPage() {
  const [tab, setTab] = React.useState<TabValue>("general");

  return (
    <AmpolPortalShell activeNavId="accounts-list">
      <Breadcrumb className="mb-density-md">
        <BreadcrumbList className="text-density-sm text-ampol-gray-600">
          <BreadcrumbItem>
            <BreadcrumbLink asChild>
              <Link href={HOME_HREF} className="text-ampol-blue-600 hover:text-ampol-blue-700">
                Home
              </Link>
            </BreadcrumbLink>
          </BreadcrumbItem>
          ...
          <BreadcrumbItem>
            <BreadcrumbLink asChild>
              <Link
                href="/pages/ci-portal/ampol/accounts"
                className="text-ampol-blue-600 hover:text-ampol-blue-700"
              >
                Accounts
              </Link>
            </BreadcrumbLink>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbPage className="text-ampol-gray-600">
              Account details
            </BreadcrumbPage>
          </BreadcrumbItem>
        </BreadcrumbList>
