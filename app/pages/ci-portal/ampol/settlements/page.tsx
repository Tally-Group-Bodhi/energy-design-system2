...
function SettlementWeekCard({ card }: { card: SettlementCard }) {
  return (
    <Link
      ...
      className="block rounded-density-lg border border-ampol-gray-200 bg-white p-density-lg transition-colors hover:border-ampol-red-600/40"
    >
      <div className="mb-density-md flex items-start gap-density-md">
        <span className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-ampol-gray-100 text-density-sm font-semibold text-ampol-gray-600">
          {card.code}
        </span>
        <div className="min-w-0 flex-1">
          <p className="text-density-base font-semibold text-ampol-blue-700">
            {card.title}
          </p>
          <p className="text-density-sm text-ampol-blue-600">{card.due}</p>
        </div>
        {card.complete && (
          <span className="grid h-6 w-6 place-items-center rounded-full bg-[var(--color-ampol-success)] text-white">
            <Icon name="check" size={ICON_SM} />
          </span>
        )}
      </div>
      <div className="space-y-density-xs text-density-sm text-ampol-gray-600">
        <p className="tabular-nums">
          {card.caseId} | {card.period}
        </p>
        <p>
          Settlement:{" "}
          <span className="tabular-nums text-ampol-gray-900">
    ...
    <div className="grid items-start gap-density-xl lg:grid-cols-[auto_1fr]">
      <div>
        <Calendar
          ...
          className="ampol-calendar rounded-density-md border border-ampol-gray-200 bg-white [--cell-size:2.75rem]"
        />
        <p className="mt-density-md flex items-center gap-density-xs text-density-xs text-ampol-gray-600">
          <span
            aria-hidden="true"
            className="h-1.5 w-1.5 rounded-full bg-ampol-red-600"
          />
      ...
      <div>
        ...
        {dayCards.length > 0 ? (
          ...
          <div className="flex items-center gap-density-sm rounded-density-lg border border-dashed border-ampol-gray-300 p-density-lg text-density-sm text-ampol-gray-600">
            <Icon
              name="event_available"
              size={ICON_MD}
              className="text-ampol-gray-600"
            />
...
export default function SettlementsHomePage() {
  ...
  const [participant, setParticipant] = React.useState("AMPOL");
  const [goWeek, setGoWeek] = React.useState("2026-08-09");

  return (
    <AmpolPortalShell activeNavId="settlements-home">
      ...
      <div className="mb-density-lg flex flex-wrap items-end justify-between gap-density-lg">
        <div>
          <h1 className="text-density-3xl font-semibold text-ampol-gray-900">
            Settlements
          </h1>
        ...
        <div className="flex flex-wrap items-end gap-density-md">
          <div className="w-44">
            ...
            >
              ...
              <option value="AMPOLNRG">AMPOLNRG</option>
            </Select>
          </div>
          <button
            type="button"
            disabled
            className="inline-flex h-11 items-center gap-density-sm rounded-density-md border border-ampol-gray-300 px-density-lg text-density-sm text-ampol-gray-600 opacity-60"
          >
      ...
      <Card className="border-ampol-gray-200 bg-white">
        <CardContent className="p-density-xl pt-density-xl">
          <div
            ...
            className="mb-density-lg flex border-b border-ampol-gray-200"
          >
            {(
              [
