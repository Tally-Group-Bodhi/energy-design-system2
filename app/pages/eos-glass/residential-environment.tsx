...
/** Demo timeline — call on 2026/07/03; customer is querying the bill issued 07/01 for the May–Jun period */
const DEMO_CALL_DATE = "2026/07/03";
const DEMO_CALL_TIME = "13:30";
const DEMO_CALL_TIMESTAMP = "2026/07/03 13:30";
const DEMO_CALL_SUMMARY_TIMESTAMP = "2026/07/03 13:31";
const DEMO_ANALYSIS_MESSAGE = "Please give me a minute while I run the high bill analysis.";
const DEMO_ANALYSIS_TIMESTAMP = "2026/07/03 13:30";
const DEMO_BILL_PERIOD = "2026/05/31 – 2026/06/29";
const DEMO_BILL_PERIOD_RANGE = "2026/05/31 to 2026/06/29";
const DEMO_PLAN_CHANGE_DATE = "2026/06/06";
const DEMO_INVOICE_ISSUED = "2026/07/01";
const DEMO_INVOICE_DUE = "2026/07/22";
const DEMO_BILLED_TO = "Billed To 2026/06/29";
const DEMO_NEXT_READ = "2026/08/17";
...
/** Shibuya — stable usage then +22% spike on current bill period (338 → 412 kWh) */
const SHIBUYA_USAGE: UsagePoint[] = [
  { month: "Nov", usage: 305 },
  { month: "Dec", usage: 298 },
  { month: "Jan", usage: 312 },
  { month: "Feb", usage: 305 },
...
/** Shibuya — demo account: ¥14,074 bill, +22% vs ¥11,542 previous (338 → 412 kWh) */
const SHIBUYA_BILL_VS_PREVIOUS: BillVsPreviousPoint[] = [
...
/** Kamakura — ¥6,820 bill, 8% down vs ~¥7,400 previous */
const KAMAKURA_BILL_VS_PREVIOUS: BillVsPreviousPoint[] = [
...
/** Hachioji — ¥9,215 bill, 15% up vs ~¥8,000 previous */
const HACHIOJI_BILL_VS_PREVIOUS: BillVsPreviousPoint[] = [
...
const ACCOUNT_CHART_DATA: Record<string, AccountChartData> = {
  ...
  "3-18-1 Meieki, Nakamura-ku, Nagoya, Aichi 450-0002": {
    billVsPrevious: SHIBUYA_BILL_VS_PREVIOUS,
  ...
  "2-9-4 Minatomirai, Nishi-ku, Yokohama, Kanagawa 220-0012": {
    billVsPrevious: SHIBUYA_BILL_VS_PREVIOUS,
  ...
  "1-2-6 Hakata-eki-mae, Hakata-ku, Fukuoka 812-0011": {
    billVsPrevious: SHIBUYA_BILL_VS_PREVIOUS,
...
const BILL_COMPARE_BILLS = [
  {
    ...
    adoraColor: "#F97316",
    adoraSummary:
      `Invoice 33066423 for new charges of ¥14,074 (${DEMO_BILL_PERIOD_RANGE}, 30 days). Usage increased 22% compared to the previous invoice. Plan changed to Standard Plan L on ${DEMO_PLAN_CHANGE_DATE}.`,
  },
  {
    id: "bill-2",
    label: "Bill 2",
    period: "2026/04/30 – 2026/05/29",
    status: "Paid" as const,
    adoraColor: "#EF4444",
    adoraSummary: "Previous bill received a ¥7,500 government energy subsidy credit.",
  },
  {
    id: "bill-3",
    label: "Bill 3",
    period: "2026/03/31 – 2026/04/29",
    status: "Paid" as const,
    adoraColor: "#EF4444",
    adoraSummary: "Baseline period before plan change to Standard Plan L.",
  },
];

const BILL_COMPARE_ROWS: { label: string; values: string[]; detail?: string }[] = [
  { label: "Invoice issue date", values: [DEMO_INVOICE_ISSUED, "2026/06/01", "2026/05/01"] },
  { label: "Energy plan", values: ["Standard Plan L", "Standard Plan L", "Standard Plan S"], detail: `Plan changed to Standard Plan L on ${DEMO_PLAN_CHANGE_DATE}` },
  { label: "Invoice amount", values: ["¥14,074", "¥11,542", "¥10,890"] },
  ...
  { label: "General usage charge", values: ["¥9,840", "¥8,120", "¥7,650"] },
  { label: "Basic monthly charge", values: ["¥4,234", "¥4,234", "¥4,234"] },
  { label: "Total new charges", values: ["¥14,074", "¥11,542", "¥10,890"] },
  { label: "Consumption tax (10%)", values: ["¥1,279", "¥1,049", "¥990"] },
  { label: "Previous Balance", values: ["−¥6,914 CR", "¥0", "−¥1,250 CR"] },
  { label: "Government energy subsidy credit", values: ["—", "−¥7,500", "−¥7,500"], detail: "No government energy subsidy credit on current bill" },
  { label: "Hardship credit", values: ["−¥10,000", "—", "—"] },
  { label: "Carried forward credit", values: ["−¥6,914 CR", "−¥4,520 CR", "−¥1,250 CR"] },
];
...
const FUEL_META: Record<string, { icon: string; className: string }> = {
  ...
  Electric: { icon: "bolt", className: "text-sky-500 dark:text-sky-400" },
...
function AccountFuelIcons({
  ...
  return (
    ...
    >
      {parts.map((part) => {
        const meta = FUEL_META[part] ?? FUEL_META.Electric;
...
const CUSTOMER = {
  ...
  dob: "1964/08/02",
  age: 61,
  email: "noreply3@tally-group.co.jp",
  phone: "090-4170-0012",
  preference: "Contact via Phone",
  marketing: "No Marketing",
  identity: "My Number Card",
  payerRating: "Inconsistent",
};

const ACCOUNTS: AccountRecord[] = [
  {
    address: "3-14-2 SHIBUYA, SHIBUYA-KU, TOKYO 150-0002",
    nmi: "0311011200050311010991",
    accNumber: "000011420511",
    type: "Residential",
    fuel: "Electric",
    status: "OPEN",
    balance: "−¥6,914 Credit",
    isCredit: true,
    tags: [
      { label: "Hardship", variant: "hardship" },
      { label: "Government Assist", variant: "government" },
    ],
    plan: "Standard Plan L",
    ...
    commenced: "2019/03/14",
    invoiceAmount: "¥14,074",
  ...
  {
    address: "1-7-5 KAMAKURAYAMA, KAMAKURA-SHI, KANAGAWA 248-0031",
    nmi: "0311011200060424105902",
    accNumber: "000011420512",
    type: "Residential",
    fuel: "Electric + Solar",
    status: "OPEN",
    balance: "−¥2,450 Credit",
    isCredit: true,
    plan: "Standard Plan S",
    bestOffer: "Currently on best offer",
    billing: "MONTHLY Billing",
    billingTo: "Billed To 2026/06/15",
    scheduledRead: "2026/08/08",
    billPeriod: "2026/05/16 – 2026/06/15",
    commenced: "2021/09/02",
    invoiceAmount: "¥6,820",
    allocated: "Fully Allocated",
    posted: "2026/06/18",
    due: "2026/07/08",
  ...
  {
    address: "2-11-8 MINAMIOSAWA, HACHIOJI-SHI, TOKYO 192-0364",
    nmi: "0311011200073012107903",
    accNumber: "000011420513",
    type: "Residential",
    fuel: "Electric + Gas",
    status: "OPEN",
    balance: "¥9,215",
    ...
    plan: "Smart Life Plan",
    bestOffer: "Better offer available",
    billing: "MONTHLY Billing",
    billingTo: "Billed To 2026/06/22",
    scheduledRead: "2026/08/14",
    billPeriod: "2026/05/23 – 2026/06/22",
    commenced: "2017/06/11",
    invoiceAmount: "¥9,215",
  ...
  {
    address: "4-2-9 GINZA, CHUO-KU, TOKYO 104-0061",
    nmi: "0311011200083092101324",
    accNumber: "000011420621",
    type: "Small Business",
    fuel: "Electric",
    status: "OPEN",
    balance: "−¥6,730 Credit",
    isCredit: true,
    plan: "Business Denki Plan S",
    bestOffer: "Currently on best offer",
    billing: "MONTHLY Billing",
    billingTo: "Billed To 2026/06/20",
    scheduledRead: "2026/08/12",
    billPeriod: "2026/05/21 – 2026/06/20",
    commenced: "2020/02/08",
    invoiceAmount: "¥14,280",
    allocated: "Fully Allocated",
    posted: "2026/06/22",
    due: "2026/07/10",
    charges: "¥14,280",
  ...
  {
    address: "1-15-3 NAKAMEGURO, MEGURO-KU, TOKYO 153-0061",
    nmi: "0311011200093101101435",
    accNumber: "000011420622",
    type: "Small Business",
    fuel: "Electric + Gas",
    status: "OPEN",
    balance: "¥21,450",
    isCredit: false,
    plan: "Business Denki Plan L",
    bestOffer: "Better offer available",
    billing: "MONTHLY Billing",
    billingTo: "Billed To 2026/06/18",
    scheduledRead: "2026/08/10",
    billPeriod: "2026/05/19 – 2026/06/18",
    commenced: "2018/11/19",
    invoiceAmount: "¥21,450",
    allocated: "Partially Allocated",
    posted: "2026/06/20",
    due: "2026/07/09",
    charges: "¥21,450",
  ...
  {
    address: "3-6-12 KOENJI-MINAMI, SUGINAMI-KU, TOKYO 166-0003",
    nmi: "0311011200102103204416",
    accNumber: "000011420623",
    type: "Small Business",
    fuel: "Electric",
    status: "OPEN",
    balance: "−¥1,890 Credit",
    isCredit: true,
    plan: "Business Denki Plan S",
    bestOffer: "Currently on best offer",
    billing: "MONTHLY Billing",
    billingTo: "Billed To 2026/06/25",
    scheduledRead: "2026/08/16",
    billPeriod: "2026/05/26 – 2026/06/25",
    commenced: "2022/04/03",
    invoiceAmount: "¥9,840",
    allocated: "Fully Allocated",
    posted: "2026/06/27",
    due: "2026/07/15",
  ...
  {
    address: "2-1-1 Nishi-Shinjuku, Shinjuku-ku, Tokyo 163-0811",
    nmi: "0311011200114021901127",
    accNumber: "000011430101",
    type: "Commercial",
    fuel: "Electric + Gas",
    status: "OPEN",
    balance: "¥28,400,000",
    isCredit: false,
    plan: "High-Voltage Flex 2026",
    bestOffer: "Review recommended",
    billing: "MONTHLY Billing",
    billingTo: "Billed To 2026/04/12",
    scheduledRead: "2026/06/12",
    billPeriod: "2026/03/13 – 2026/04/12",
    commenced: "2015/01/15",
    invoiceAmount: "¥28,400,000",
    ...
    billVsPreviousUp: true,
    ciDetailHref:
      "/pages/eos-glass?segment=commercial&expanded=true&view=customer&customer=metro-retail&site=site-shinjuku",
  },
  {
    address: "1-4-8 Umeda, Kita-ku, Osaka 530-0001",
    nmi: "0411011200125038209208",
    accNumber: "000011430102",
    type: "Commercial",
    fuel: "Gas",
    status: "OPEN",
    balance: "¥17,100,000",
    isCredit: false,
    plan: "Low-Voltage Portfolio Fixed 2026",
    bestOffer: "Currently on best offer",
    billing: "MONTHLY Billing",
    billingTo: "Billed To 2026/05/18",
    scheduledRead: "2026/07/18",
    billPeriod: "2026/04/19 – 2026/05/18",
    commenced: "2016/08/22",
    invoiceAmount: "¥17,100,000",
    allocated: "Unbilled exposure",
    posted: "2026/05/18",
    due: "—",
    charges: "¥17,100,000",
    ...
    billVsPreviousUp: true,
    ciDetailHref:
      "/pages/eos-glass?segment=commercial&expanded=true&view=customer&customer=gas-parent&site=site-umeda",
  },
  {
    address: "3-18-1 Meieki, Nakamura-ku, Nagoya, Aichi 450-0002",
    nmi: "0511011200136049310319",
    accNumber: "000011430103",
    type: "Commercial",
    fuel: "Electric + Gas",
    status: "OPEN",
    balance: "¥13,200,000",
    ...
    bestOffer: "Review recommended",
    billing: "MONTHLY Billing",
    billingTo: "Billed To 2026/04/09",
    scheduledRead: "2026/06/09",
    billPeriod: "2026/03/10 – 2026/04/09",
    commenced: "2014/03/09",
    invoiceAmount: "¥13,200,000",
    allocated: "Unbilled exposure",
    posted: "2026/04/09",
    due: "—",
    charges: "¥13,200,000",
    ...
    billVsPreviousUp: true,
    ciDetailHref:
      "/pages/eos-glass?segment=commercial&expanded=true&view=customer&customer=metro-retail&site=site-nagoya",
  },
  {
    address: "2-9-4 Minatomirai, Nishi-ku, Yokohama, Kanagawa 220-0012",
    nmi: "0311011200147051411420",
    accNumber: "000011430104",
    type: "Commercial",
    fuel: "Electric",
    status: "OPEN",
    balance: "¥8,600,000",
    ...
    bestOffer: "Currently on best offer",
    billing: "MONTHLY Billing",
    billingTo: "Billed To 2026/05/22",
    scheduledRead: "2026/07/22",
    billPeriod: "2026/04/23 – 2026/05/22",
    commenced: "2019/07/01",
    invoiceAmount: "¥8,600,000",
    allocated: "Unbilled exposure",
    posted: "2026/05/22",
    due: "—",
    charges: "¥8,600,000",
    ...
    billVsPreviousUp: false,
    ciDetailHref:
      "/pages/eos-glass?segment=commercial&expanded=true&view=customer&customer=metro-retail&site=site-yokohama",
  },
  {
    address: "1-2-6 Hakata-eki-mae, Hakata-ku, Fukuoka 812-0011",
    nmi: "0811011200158062512531",
    accNumber: "000011430105",
    type: "Commercial",
    fuel: "Gas",
    status: "OPEN",
    balance: "¥11,800,000",
    ...
    bestOffer: "Currently on best offer",
    billing: "MONTHLY Billing",
    billingTo: "Billed To 2026/05/20",
    scheduledRead: "2026/07/20",
    billPeriod: "2026/04/21 – 2026/05/20",
    commenced: "2024/01/01",
    invoiceAmount: "¥11,800,000",
    allocated: "Unbilled exposure",
    posted: "2026/05/20",
    due: "—",
    charges: "¥11,800,000",
    ...
    billVsPreviousUp: true,
    ciDetailHref:
      "/pages/eos-glass?segment=commercial&expanded=true&view=customer&customer=community-gas&site=site-hakata",
  },
];
...
function AccountBillPanels({ acc, chartId }: { acc: AccountRecord; chartId: string }) {
  const accountCharts = getAccountChartData(acc.address);
  const [invoicePreviewOpen, setInvoicePreviewOpen] = useState(false);

  const chargesRows = acc.isClosed ? (
    <>
        ...
        <Tabs defaultValue="charges">
          ...
          <CardContent className="p-5 pt-4 pb-5">
            ...
            <TabsContent value="charges" className="mt-4">
              <div className="space-y-3 text-sm text-gray-700 dark:text-slate-300">{chargesRows}</div>
            </TabsContent>
            ...
            <TabsContent value="usage" className="mt-4">
              ...
              />
              <AccountUsageBarChart data={accountCharts.usage} gradientId={`gradUsage-${chartId}`} />
            </TabsContent>
          </CardContent>
        </Tabs>
      </Card>
          ...
          <CardContent className="p-5 pt-4 pb-5">
            <TabsContent value="invoice-pdf" className="mt-0">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h3 className="text-sm font-semibold tracking-tight text-gray-900 dark:text-slate-100">Latest invoice</h3>
              ...
              <div className="mt-4 grid grid-cols-2 gap-3">
                <div className="rounded-xl border border-gray-200/80 bg-white p-3 dark:border-white/10 dark:bg-white/[0.04]">
                  <p className="text-[10px] font-medium uppercase tracking-wider text-gray-400 dark:text-slate-500">Amount due</p>
                  <p className="mt-1 text-lg font-semibold text-gray-900 dark:text-slate-100">
                    {acc.invoiceAmount ?? acc.finalInvoice ?? "—"}
                  </p>
                </div>
                <div className="rounded-xl border border-gray-200/80 bg-white p-3 dark:border-white/10 dark:bg-white/[0.04]">
                  <p className="text-[10px] font-medium uppercase tracking-wider text-gray-400 dark:text-slate-500">Due date</p>
            ...
            <TabsContent value="bill-vs-previous" className="mt-0">
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="rounded-lg border border-gray-200 bg-gray-50 px-2 py-1.5 text-xs font-medium text-gray-700 dark:border-white/10 dark:bg-white/5 dark:text-slate-300">
                  Latest bill
                </span>
              </div>
              ...
              <div className="mt-2 h-52 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <ComposedChart data={accountCharts.billVsPrevious} margin={{ top: 12, right: 12, bottom: 4, left: -8 }}>
                    <defs>
                      <linearGradient id={`bill-area-${chartId}`} x1="0" y1="0" x2="0" y2="1">
...
const ACCOUNT_USAGE_SPARKLINES: Record<string, number[]> = {
  "3-14-2 SHIBUYA, SHIBUYA-KU, TOKYO 150-0002": [305, 298, 312, 305, 318, 328, 338, 412],
  "1-7-5 KAMAKURAYAMA, KAMAKURA-SHI, KANAGAWA 248-0031": [95, 88, 92, 85, 78, 74, 72, 68],
  "2-11-8 MINAMIOSAWA, HACHIOJI-SHI, TOKYO 192-0364": [185, 192, 178, 185, 198, 215, 228, 210],
  "4-2-9 GINZA, CHUO-KU, TOKYO 104-0061": [160, 155, 148, 152, 145, 140, 138, 132],
  "1-15-3 NAKAMEGURO, MEGURO-KU, TOKYO 153-0061": [210, 218, 225, 220, 235, 242, 248, 255],
  "3-6-12 KOENJI-MINAMI, SUGINAMI-KU, TOKYO 166-0003": [120, 118, 115, 112, 110, 108, 105, 102],
  "2-1-1 Nishi-Shinjuku, Shinjuku-ku, Tokyo 163-0811": [820, 840, 860, 855, 880, 900, 920, 945],
  "1-4-8 Umeda, Kita-ku, Osaka 530-0001": [410, 420, 430, 425, 440, 450, 460, 470],
  "3-18-1 Meieki, Nakamura-ku, Nagoya, Aichi 450-0002": [650, 670, 690, 680, 710, 730, 745, 760],
  "2-9-4 Minatomirai, Nishi-ku, Yokohama, Kanagawa 220-0012": [280, 275, 270, 268, 265, 260, 258, 255],
  "1-2-6 Hakata-eki-mae, Hakata-ku, Fukuoka 812-0011": [140, 148, 155, 162, 170, 178, 185, 192],
};

const CUSTOMER_SUMMARY =
  `Invoice 33066423 for new charges of ¥14,074 (${DEMO_BILL_PERIOD_RANGE}, 30 days). The previous bill received a ¥7,500 government energy subsidy credit. Usage increased 22% compared to the previous invoice. Plan changed to Standard Plan L on ${DEMO_PLAN_CHANGE_DATE}, which will also affect future bills. A ¥10,000 hardship credit was applied to this bill's balance. Meter readings for this period were based on actual usage. The account has a solar system with a FIT feed-in tariff.`;

const ADORA_OVERVIEW_SEGMENTS = [
  { text: "Invoice ", bold: false },
  { text: "33066423", bold: true },
  { text: " for new charges of ", bold: false },
  { text: "¥14,074", bold: true },
  { text: ` (${DEMO_BILL_PERIOD_RANGE}, 30 days). The previous bill received a `, bold: false },
  { text: "¥7,500 government energy subsidy credit", bold: true },
  { text: ". Usage increased ", bold: false },
  { text: "22%", bold: true },
  { text: " compared to the previous invoice. Plan changed to ", bold: false },
  { text: "Standard Plan L", bold: true },
  { text: ` on ${DEMO_PLAN_CHANGE_DATE}, which will also affect future bills. A `, bold: false },
  { text: "¥10,000 hardship credit", bold: true },
  ...
  { text: "solar system with a FIT feed-in tariff", bold: true },
  { text: ".", bold: false },
];
...
const PRIMARY_ACCOUNT_ADDRESS = "3-14-2 SHIBUYA, SHIBUYA-KU, TOKYO 150-0002";

const CALL_DEMO_SECTIONS = [
  {
    title: "High Bill Suggestions:",
    items: [
      `Invoice 33066423 for new charges of ¥14,074 (${DEMO_BILL_PERIOD_RANGE}, 30 days)`,
      "Previous bill received a ¥7,500 government energy subsidy credit",
      "Usage increased 22% compared to the previous invoice",
      `Plan changed to Standard Plan L on ${DEMO_PLAN_CHANGE_DATE} (also affects future bills)`,
    ],
  },
  {
    title: "Additional Info:",
    items: ["A ¥10,000 hardship credit was applied to this bill's balance"],
  },
  {
    title: "Supplementary Context:",
    items: [
      "Meter readings for this period were based on actual usage",
      "The account has a solar system with a FIT feed-in tariff",
    ],
  },
] as const;

const CALL_DEMO_SUMMARY =
  "Your recent bill is higher due to new charges of ¥14,074, a previous government energy subsidy credit of ¥7,500, a 22% increase in usage, and a plan change to Standard Plan L. A ¥10,000 hardship credit was applied to this bill's balance.";

const CALL_WRAP_NOTE_REF = "INT-20260703-0142";

const CALL_WRAP_SUMMARY_POINTS = [
  "Customer called regarding a higher-than-expected bill for invoice 33066423 (¥14,074, 2026/05/31 – 2026/06/29).",
  "Explained that the previous bill included a ¥7,500 government energy subsidy credit not applied to the current invoice.",
  "Reviewed 22% usage increase compared to the previous billing period and plan change to Standard Plan L on 2026/06/06.",
  "Confirmed ¥10,000 hardship credit already applied to the current bill balance.",
  "Customer acknowledged the explanation; no further escalation required at this time.",
] as const;

const DEMO_ANALYSE_DELAY = 3500;

interface ResidentialEnvironmentProps {
  environment: EosGlassEnvironment;
  onEnvironmentChange: (next: EosGlassEnvironment) => void;
}

export default function ResidentialEnvironment(props: ResidentialEnvironmentProps) {
  return (
    <Suspense>
      <ResidentialEnvironmentContent {...props} />
    </Suspense>
  );
}

function ResidentialEnvironmentContent({
  environment,
  onEnvironmentChange,
}: ResidentialEnvironmentProps) {
  ...
  return (
    ...
    >
      {/* Seamless chrome: header + nav as one iOS-white block (no border between them) */}
      <div className="flex min-h-0 flex-1 flex-col">
        ...
        <div className="flex flex-1 min-h-0">
          {/* Collapsed nav — same iOS-white block as header, no seam */}
          ...
          <div className={cn("relative shrink-0 border-t p-2 flex flex-col items-center gap-0.5", IOS_CHROME_BORDER_CLASS)}>
            ...
            {displayOptionsOpen && (
              <>
                ...
                <div
                  className={cn(
                    "absolute bottom-0 left-full z-50 ml-3 w-56 rounded-xl border p-4 shadow-2xl",
                    IOS_CHROME_BORDER_CLASS,
                    GLASS_CARD_LIGHT,
                    GLASS_CARD_DARK
                  )}
              ...
              <div className="space-y-2">
                ...
                <Card className={cn("overflow-hidden border-0", GLASS_CARD_LIGHT, GLASS_CARD_DARK)}>
                  <CardContent className="p-4 pt-4">
                    ...
                    <div className="space-y-2.5 text-xs">
                      {[
                        { icon: "chat_bubble_outline" as const, label: "Created", date: "2008/04/05" },
                        { icon: "call" as const, label: "Last call", date: "2026/06/18" },
                        { icon: "description" as const, label: "Last note", date: "2026/06/20" },
                        { icon: "computer" as const, label: "Last web", date: "2026/06/20" },
                      ].map((row) => (
                ...
                <Card className={cn("overflow-hidden border-0", GLASS_CARD_LIGHT, GLASS_CARD_DARK)}>
                  <CardContent className="p-4 pt-4">
                    ...
                    <div className="space-y-3 text-xs">
                      ...
                      <div className="flex items-center gap-2.5">
                        <div className="flex h-6 w-9 shrink-0 items-center justify-center rounded bg-[#0B4EA2] text-[8px] font-bold tracking-wide text-white">JCB</div>
                        <div className="min-w-0 flex-1">
                          <div className="font-medium text-gray-800 dark:text-slate-200">.... 1234</div>
                          <div className="text-[10px] text-gray-400 dark:text-slate-600">Exp 06/2027</div>
                        </div>
                      ...
                      <div className="flex items-center gap-2.5">
                        <div className="flex h-6 w-9 shrink-0 items-center justify-center rounded bg-violet-600 text-[8px] font-bold text-white">口座</div>
                        <div className="min-w-0 flex-1">
                          <div className="font-medium text-gray-800 dark:text-slate-200">Account Transfer</div>
                          <div className="text-[10px] text-gray-400 dark:text-slate-600">MUFG Bank · Shibuya</div>
                        </div>
                      </div>
          ...
          <section>
            ...
            <div className="space-y-8">
              {ACCOUNT_SECTIONS.map((section) => {
                ...
                const balancePillGreen = acc.isCredit || acc.balance === "¥0";
                ...
                return (
                  ...
                  <AccountBillPanels acc={acc} chartId={acc.nmi.replace(/\s/g, "")} />
                        </div>
                      )}
                    </Card>
                  </React.Fragment>
                      ...
                      <div className={cn(isSelected ? "flex gap-6" : "")}>
                        ...
                        <div className={cn(isSelected ? "w-72 shrink-0" : "")}>
                          ...
                          <div className="px-5 pt-5 pb-3">
                            <div className="flex items-start justify-between mb-3">
                              <AccountFuelIcons fuel={acc.fuel} size={19} className="w-10" />
                              <div className="flex items-center gap-2">
                                <span
                                  className={cn(
                            ...
                            <p className="mt-0.5 text-xs text-gray-500 dark:text-slate-500">
                              SPID: {acc.nmi}
                              {acc.accNumber ? ` · BA#: ${acc.accNumber}` : ""}
                            </p>
                          </div>
                        ...
                        {isSelected && (
                          ...
                          >
                            ...
                            <AccountBillPanels acc={acc} chartId={`card-${acc.nmi.replace(/\s/g, "")}`} />
                          </div>
                        )}
                      </div>
                    </CardContent>
            ...
            <TabsContent value="insights" className="mt-0">
              ...
              <div className="mb-3 grid grid-cols-2 gap-2 lg:grid-cols-4">
                {[
                  ...
                  { icon: "payments" as const, label: "Latest Bill", value: "¥14,074 / month", color: "#0091BF" },
                    ...
                    <div className="h-56 w-full">
                      <ResponsiveContainer width="100%" height="100%">
                        <ComposedChart data={INSIGHT_USAGE_DATA} margin={{ top: 5, right: 10, left: -15, bottom: 0 }}>
                          ...
                          <Bar yAxisId="right" dataKey="cost" fill="#E8EBED" radius={[4, 4, 0, 0]} name="Cost (¥ hundreds)" barSize={20} />
                        </ComposedChart>
                      </ResponsiveContainer>
                    </div>
                  </CardContent>
              ...
              <div className="grid grid-cols-2 gap-2 lg:grid-cols-4">
                {[
                  { num: "¥112,100", label: "Total Billed (12mo)", change: "↓ 34% vs prior year", changeColor: "text-emerald-600 dark:text-emerald-400" },
                  ...
                  { num: "16.8", label: "Years as Customer", change: "Since 2008/04", changeColor: "text-[#0091BF] dark:text-[#00C1FF]" },
                ].map((stat) => (
              ...
              {!controlPanelOpen ? (
                <div className="flex h-full flex-col items-center">
                  ...
                  >
                    <span
                      className={cn(
                        "block h-full w-full rounded-tr-xl",
                        PANE_LIGHT,
                        PANE_DARK
                      )}
                    />
                ...
                {activePanelTab === "X-Sell" && xSellView === null && (
                  <div className="flex-1 space-y-5 overflow-y-auto px-3.5 py-3.5">
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {([
                          { icon: "bolt" as const, iconBg: "bg-amber-100 text-amber-700 dark:bg-amber-500/25 dark:text-amber-300", service: "Electric", provider: "Marubeni Power Retail", plan: "Standard Plan L", status: "Connected" },
                          { icon: "local_fire_department" as const, iconBg: "bg-orange-100 text-orange-700 dark:bg-orange-500/25 dark:text-orange-300", service: "Gas", provider: "Marubeni Power Retail", plan: "Online Gas Saver 2026", status: "Connected" },
                        ]).map((s) => (
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {([
                          { icon: "wifi" as const, iconBg: "bg-sky-100 text-sky-700 dark:bg-sky-500/25 dark:text-sky-300", service: "Fibre", headline: "6 fibre broadband plans available", sub: "Plans from ¥6,800/month", badge: null, action: "broadband" },
