import PageBanner from "@/components/PageBanner/PageBanner";

/** Tally Group official logo (TallyOfficialLogo.svg) – 397×76 */
function TallyGroupLogo({
  blueFill = "#2C365D",
  tealFill = "#4EEECA",
  className = "h-12 w-auto",
}: {
  blueFill?: string;
  tealFill?: string;
  className?: string;
}) {
  return (
    <svg
      width="397"
      height="76"
      viewBox="0 0 397 76"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      aria-label="Tally Group logo"
    >
      <path
        d="M82.9216 12.4233C88.9339 12.4233 93.3818 13.9264 96.2652 16.9325C99.21 19.9386 100.682 24.5092 100.682 30.6442V58.4356H91.756V50.9816C90.5904 53.6196 88.8419 55.6442 86.5106 57.0552C84.2407 58.4049 81.572 59.0797 78.5045 59.0797C75.5597 59.0797 72.8603 58.4969 70.4063 57.3313C67.9523 56.1043 66.0198 54.4172 64.6088 52.2699C63.2591 50.1227 62.5842 47.73 62.5842 45.092C62.5842 41.8405 63.4124 39.2945 65.0689 37.454C66.7867 35.5521 69.5781 34.2024 73.4431 33.4049C77.3695 32.6074 82.7376 32.2086 89.5474 32.2086H91.664V29.5399C91.664 26.1043 90.9278 23.6196 89.4554 22.0859C88.0443 20.5521 85.7131 19.7853 82.4615 19.7853C77.3695 19.7853 72.2775 21.3497 67.1855 24.4785L64.4247 17.9448C66.8174 16.2883 69.7008 14.9693 73.075 13.9877C76.5106 12.9448 79.7928 12.4233 82.9216 12.4233ZM80.1609 52.1779C83.5351 52.1779 86.2959 51.0429 88.4431 48.773C90.5904 46.4417 91.664 43.4662 91.664 39.8466V37.454H90.0075C85.2223 37.454 81.5413 37.6687 78.9646 38.0981C76.3879 38.5276 74.5474 39.2638 73.4431 40.3067C72.3388 41.2883 71.7867 42.7301 71.7867 44.6319C71.7867 46.7791 72.5536 48.5889 74.0873 50.0613C75.6824 51.4724 77.7069 52.1779 80.1609 52.1779Z"
        fill={blueFill}
      />
      <path
        d="M57.1472 51.9018C58.5583 51.9018 59.9386 51.8098 61.2883 51.6258V58.7117C59.7546 58.9571 57.6994 59.0797 56.0429 59.0797C50.2147 59.0797 45.8589 57.6687 42.9755 54.8466C40.1534 51.9632 38.7423 47.8221 38.7423 42.4233V20.7055H30V13.5276H38.7423V0H48.0368V13.5276H60.3681V20.7055H48.0368V42.0552C48.0368 48.6196 51.0736 51.9018 57.1472 51.9018Z"
        fill={blueFill}
      />
      <path d="M105.711 0H115.006V58.4356H105.711V0Z" fill={blueFill} />
      <path d="M120.352 0H129.646V58.4356H120.352V0Z" fill={blueFill} />
      <path
        d="M179.691 13.6196L152.911 75H143.525L151.347 57.3313L132.482 13.6196H142.237L156.224 47.9448L170.396 13.6196H179.691Z"
        fill={blueFill}
      />
      <path
        d="M219 14.083V55.6041C219 62.3426 217.257 67.4265 213.772 70.8559C210.344 74.2853 205.261 76 198.523 76C194.863 76 191.378 75.4886 188.066 74.4658C184.813 73.5032 181.909 72.0893 179.353 70.2242L182.054 63.6361C184.842 65.5012 187.515 66.8248 190.071 67.607C192.685 68.4493 195.473 68.8705 198.436 68.8705C206.394 68.8705 210.373 64.689 210.373 56.3261V49.377C209.154 52.0243 207.295 54.1 204.797 55.6041C202.299 57.048 199.394 57.77 196.083 57.77C192.365 57.77 189.054 56.8675 186.149 55.0626C183.245 53.2576 180.979 50.7006 179.353 47.3916C177.784 44.0825 177 40.262 177 35.9301C177 31.5983 177.813 27.7778 179.44 24.4687C181.066 21.1596 183.303 18.6026 186.149 16.7977C189.054 14.9326 192.365 14 196.083 14C199.336 14 202.183 14.722 204.622 16.1659C207.12 17.6099 209.008 19.6555 210.286 22.3028V14.083H219ZM198.174 50.5502C201.95 50.5502 204.913 49.2567 207.062 46.6696C209.212 44.0825 210.286 40.5027 210.286 35.9301C210.286 31.4178 209.212 27.868 207.062 25.2809C204.971 22.6337 202.008 21.31 198.174 21.31C194.34 21.31 191.32 22.6036 189.112 25.1907C186.963 27.7778 185.888 31.3576 185.888 35.9301C185.888 40.5027 186.992 44.0825 189.199 46.6696C191.407 49.2567 194.398 50.5502 198.174 50.5502Z"
        fill={tealFill}
      />
      <path
        d="M249.513 19.8661L244.261 20.3828C240.128 20.7846 237.143 22.0475 235.306 24.1715C233.526 26.2955 232.636 28.9935 232.636 32.2655V58.6005H223.94V13.5803H232.378V20.8994C234.617 15.9626 239.181 13.2359 246.069 12.7193L248.911 12.5471L249.513 19.8661Z"
        fill={tealFill}
      />
      <path
        d="M270.5 59C266.4 59 262.784 58.0625 259.652 56.1874C256.577 54.3123 254.185 51.6509 252.477 48.2032C250.826 44.6949 250 40.6423 250 36.0454C250 31.3879 250.826 27.3353 252.477 23.8876C254.185 20.3794 256.577 17.6877 259.652 15.8126C262.784 13.9375 266.4 13 270.5 13C274.6 13 278.188 13.9375 281.263 15.8126C284.394 17.6877 286.786 20.3794 288.438 23.8876C290.146 27.3353 291 31.3879 291 36.0454C291 40.6423 290.146 44.6949 288.438 48.2032C286.786 51.6509 284.394 54.3123 281.263 56.1874C278.188 58.0625 274.6 59 270.5 59ZM270.5 51.7416C274.315 51.7416 277.219 50.4109 279.213 47.7495C281.263 45.0881 282.288 41.1867 282.288 36.0454C282.288 30.9645 281.263 27.0934 279.213 24.432C277.163 21.7101 274.258 20.3491 270.5 20.3491C266.742 20.3491 263.838 21.7101 261.788 24.432C259.738 27.0934 258.713 30.9645 258.713 36.0454C258.713 41.1867 259.709 45.0881 261.702 47.7495C263.752 50.4109 266.685 51.7416 270.5 51.7416Z"
        fill={tealFill}
      />
      <path
        d="M333.319 13.5804V58.6005H324.881V51.798C323.561 54.209 321.724 56.046 319.37 57.3089C317.074 58.5718 314.491 59.2032 311.62 59.2032C301.402 59.2032 296.293 53.5776 296.293 42.3263V13.5804H304.99V42.2402C304.99 45.6271 305.679 48.1242 307.057 49.7315C308.435 51.3388 310.558 52.1425 313.429 52.1425C316.816 52.1425 319.514 51.0518 321.523 48.8704C323.589 46.689 324.623 43.7901 324.623 40.1736V13.5804H333.319Z"
        fill={tealFill}
      />
      <path
        d="M361.426 14C365.021 14 368.216 14.926 371.013 16.778C373.866 18.63 376.063 21.2343 377.603 24.5911C379.201 27.9478 380 31.8254 380 36.2239C380 40.5645 379.23 44.3842 377.689 47.6831C376.148 50.982 373.951 53.5285 371.098 55.3226C368.302 57.1167 365.078 58.0138 361.426 58.0138C358.287 58.0138 355.52 57.3482 353.123 56.0171C350.727 54.6281 348.9 52.6603 347.645 50.1138V75H339V14.0417H347.645V21.9867C348.9 19.4402 350.727 17.4725 353.123 16.0835C355.52 14.6945 358.287 14 361.426 14ZM359.372 51.0688C363.138 51.0688 366.048 49.7955 368.102 47.249C370.157 44.6447 371.184 40.9696 371.184 36.2239C371.184 31.4203 370.128 27.6874 368.017 25.0251C365.962 22.3629 363.081 21.0318 359.372 21.0318C355.605 21.0318 352.695 22.334 350.641 24.9383C348.644 27.5427 347.645 31.2467 347.645 36.0503C347.645 40.8539 348.644 44.5579 350.641 47.1622C352.695 49.7666 355.605 51.0688 359.372 51.0688Z"
        fill={tealFill}
      />
      <path
        d="M382.904 15.272H380.144V14H387.212V15.272H384.452V22.46H382.904V15.272Z"
        fill={tealFill}
      />
      <path
        d="M396.527 14V22.46H395.159V16.868L392.927 21.092H391.895L389.651 16.928L389.663 22.46H388.295V14H389.519L392.423 19.532L395.315 14H396.527Z"
        fill={tealFill}
      />
      <path
        d="M0 3.37143C0 1.50944 1.47598 0 3.2967 0H21.7033C23.524 0 25 1.50944 25 3.37143V5.58738L14.3812 12.9464C13.375 13.6437 12.7747 14.7901 12.7747 16.0143C12.7747 17.2385 13.375 18.3849 14.3812 19.0822L25 26.4412V32.1374L14.3812 39.4964C13.375 40.1937 12.7747 41.3401 12.7747 42.5643C12.7747 43.7885 13.375 44.9349 14.3812 45.6322L25 52.9912V55.6286C25 57.4906 23.524 59 21.7033 59H3.2967C1.47598 59 0 57.4906 0 55.6286V3.37143Z"
        fill={tealFill}
      />
    </svg>
  );
}

export default function LogoPage() {
  return (
    <>
      <PageBanner title="Logo" />

      <div className="min-h-screen bg-gray-50">
        <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
          {/* Overview */}
          <div className="mb-16">
            <h2 className="mb-4 text-2xl font-semibold tracking-tight text-gray-900">
              Overview
            </h2>
            <p className="max-w-3xl text-lg leading-7 text-gray-600">
              The Tally Group logo is our primary brand mark. It combines the
              tally wordmark with the Group wordmark and symbol. Use it
              consistently across all digital and print experiences to maintain
              brand recognition and trust.
            </p>
          </div>

          {/* Primary logo */}
          <section className="mb-16 border-t border-gray-200 pt-16">
            <h2 className="mb-4 text-2xl font-semibold tracking-tight text-gray-900">
              Primary logo
            </h2>
            <p className="mb-8 max-w-3xl text-base leading-6 text-gray-600">
              The primary Tally Group logo is the preferred version. Use it on
              light backgrounds (white or light gray). Never alter the
              proportions, colours, or spacing.
            </p>
            <div className="flex max-w-2xl min-h-[180px] items-center justify-center rounded-lg border border-gray-200 bg-white p-10">
              <TallyGroupLogo className="h-14 w-auto" />
            </div>
          </section>

          {/* Reversed logo */}
          <section className="mb-16 border-t border-gray-200 pt-16">
            <h2 className="mb-4 text-2xl font-semibold tracking-tight text-gray-900">
              Reversed logo
            </h2>
            <p className="mb-8 max-w-3xl text-base leading-6 text-gray-600">
              On dark backgrounds, use the reversed (white) version of the Tally
              Group logo. This ensures sufficient contrast and legibility.
            </p>
            <div className="flex max-w-2xl min-h-[180px] items-center justify-center rounded-lg bg-[#2C365D] p-10">
              <TallyGroupLogo
                blueFill="#FFFFFF"
                tealFill="#FFFFFF"
                className="h-14 w-auto"
              />
            </div>
          </section>

          {/* Clear space */}
          <section className="mb-16 border-t border-gray-200 pt-16">
            <h2 className="mb-4 text-2xl font-semibold tracking-tight text-gray-900">
              Clear space
            </h2>
            <p className="mb-8 max-w-3xl text-base leading-6 text-gray-600">
              Maintain clear space around the logo so it remains legible and
              uncluttered. The minimum clear space on all sides is equal to the
              height of the symbol (x). No text, graphics, or other elements
              should appear within this area.
            </p>
            <div className="relative flex max-w-2xl min-h-[200px] items-center justify-center rounded-lg border border-gray-200 bg-white p-10">
              <div className="absolute inset-10 border border-dashed border-gray-300 border-opacity-60" />
              <div className="relative">
                <TallyGroupLogo className="h-14 w-auto" />
              </div>
            </div>
            <p className="mt-4 text-sm text-gray-500">
              Dashed line indicates minimum clear space (x).
            </p>
          </section>

          {/* Minimum size */}
          <section className="mb-16 border-t border-gray-200 pt-16">
            <h2 className="mb-4 text-2xl font-semibold tracking-tight text-gray-900">
              Minimum size
            </h2>
            <p className="mb-8 max-w-3xl text-base leading-6 text-gray-600">
              To preserve legibility, do not reproduce the Tally Group logo
              smaller than these dimensions. For digital: minimum width 160px.
              For print: minimum width 25mm.
            </p>
            <div className="flex max-w-2xl flex-wrap items-end gap-12 rounded-lg border border-gray-200 bg-white p-8">
              <div>
                <p className="mb-3 text-xs font-medium uppercase tracking-wide text-gray-500">
                  Minimum (160px wide)
                </p>
                <div className="min-w-[160px]">
                  <TallyGroupLogo className="h-8 w-auto" />
                </div>
              </div>
            </div>
          </section>

          {/* Logo misuse */}
          <section className="mb-16 border-t border-gray-200 pt-16">
            <h2 className="mb-4 text-2xl font-semibold tracking-tight text-gray-900">
              Logo misuse
            </h2>
            <p className="mb-8 max-w-3xl text-base leading-6 text-gray-600">
              Do not alter the Tally Group logo in any of the following ways.
              These examples are incorrect and should never be used.
            </p>
            <div className="grid max-w-2xl gap-8 sm:grid-cols-2 lg:grid-cols-3">
              <div className="rounded-lg border border-red-200 bg-red-50/50 p-6">
                <p className="mb-4 text-sm font-semibold text-red-800">
                  Don&apos;t stretch or distort
                </p>
                <div className="flex items-center justify-center py-4">
                  <TallyGroupLogo
                    className="h-8 w-auto scale-x-150 scale-y-100"
                  />
                </div>
              </div>
              <div className="rounded-lg border border-red-200 bg-red-50/50 p-6">
                <p className="mb-4 text-sm font-semibold text-red-800">
                  Don&apos;t use unapproved colours
                </p>
                <div className="flex items-center justify-center py-4">
                  <TallyGroupLogo
                    blueFill="#00AA00"
                    tealFill="#FF6600"
                    className="h-8 w-auto"
                  />
                </div>
              </div>
              <div className="rounded-lg border border-red-200 bg-red-50/50 p-6">
                <p className="mb-4 text-sm font-semibold text-red-800">
                  Don&apos;t add effects
                </p>
                <div className="flex items-center justify-center py-4 opacity-50 blur-[1px]">
                  <TallyGroupLogo className="h-8 w-auto" />
                </div>
              </div>
            </div>
          </section>

          {/* Logo colours */}
          <section className="mb-16 border-t border-gray-200 pt-16">
            <h2 className="mb-4 text-2xl font-semibold tracking-tight text-gray-900">
              Logo colours
            </h2>
            <p className="mb-8 max-w-3xl text-base leading-6 text-gray-600">
              Use only the approved brand colours for the Tally Group logo.
              Primary blue for the tally wordmark; teal for the Group wordmark
              and symbol.
            </p>
            <div className="flex max-w-2xl flex-wrap gap-8">
              <div className="flex flex-col gap-2">
                <div
                  className="h-14 w-28 rounded-lg border border-gray-200"
                  style={{ backgroundColor: "#2C365D" }}
                />
                <span className="text-sm font-medium text-gray-900">
                  Primary blue #2C365D
                </span>
                <span className="text-xs text-gray-500">Tally wordmark</span>
              </div>
              <div className="flex flex-col gap-2">
                <div
                  className="h-14 w-28 rounded-lg border border-gray-200"
                  style={{ backgroundColor: "#4EEECA" }}
                />
                <span className="text-sm font-medium text-gray-900">
                  Teal #4EEECA
                </span>
                <span className="text-xs text-gray-500">
                  Group wordmark & symbol
                </span>
              </div>
            </div>
          </section>
        </div>
      </div>
    </>
  );
}
